...
function MemorialTreeSearchPage() {
  ...
  const fetchImage = async (imageFileName) => {
    ...
    const res = await fetch('http://autorack.proxy.rlwy.net:22941/get_memorial_image/'+imageFileName, {  
      method: "get",
  ...
  const handleOnKeyUp = async (e) => {
    ...
    if (e.key == "Enter"){
      ...
      'http://autorack.proxy.rlwy.net:22941/get_memorial_by_search_term/'+localSearchTerm, {
          method: "get",
       ...
       async function InitialFillOfTreeList(){
        if (treeList.length === 0 && firstLoad){
          ...
          let result = await fetch(
            //note: the {} is javascript tells it to interperet something as javascript and not string.
            //if used here, it works as intended. If used above, it creates problems
          'http://autorack.proxy.rlwy.net:22941/get_memorial_by_search_term/Dan Kuso The GOAT', {
              method: "get",
